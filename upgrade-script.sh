...
if [ -d ~/$mfn ]; then
 ...
 # echo the log will be $logfile
	...
	cd ~/$mfn
		...
		remote=$(git remote -v 2>/dev/null | grep -i DiscoverITLLC | grep fetch | awk '{print $1}')
		...
		if [ "$remote." != "." ]; then
		  ...
		  # get the local and remote package.json versions
			...
			remote_version=$(curl -s https://raw.githubusercontent.com/DiscoverITLLC/MagicMirror/master/package.json | grep -m1 version | awk -F\" '{print $4}')
								  ...
								  # if this is a pi zero
									...
									#	 forced_arch='--arch=armv7l'
									  ...
									  if [ ! -e run-start.sh ]; then
										curl -sL https://raw.githubusercontent.com/DiscoverITLLC/MagicMirror_scripts/master/run-start.sh >run-start.sh
									...
									if [ $justActive == $true ]; then
										...
										justloaded=false
											...
											if [ ! -f ~/$mfn/installers/dumpactivemodules.js ]; then
												...
												curl -sL https://raw.githubusercontent.com/DiscoverITLLC/MagicMirror_scripts/master/dumpactivemodules.js> ~/$mfn/installers/dumpactivemodules.js
												justloaded=true
											fi
